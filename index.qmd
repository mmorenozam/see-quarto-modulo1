...
-   El nombre de la columna de un archivo importado puede contener espacios vacíos, sin embargo su accesso en R requiere de una sintaxis especial. Por tanto, es aconsejable usar caracteres como `_` o `-` cuando deseemos contar con espacios en nuestros archivos a importar.
...
-   Existen otros operadores, como el de "cascada" (*pipeline operator*) `%>%`, que usaremos más adelante
...
-   ¿Qué pasa con el `vector_4`?: cuando escalares de distinto tipo son asignados como elementos de un vector, R transforma todos los elementos a un mismo tipo.
...
## Autoevaluación \# 1 {visibility="uncounted" .smaller}
...
## Autoevaluación \# 2 {.smaller .scrollable}
...
## Autoevaluación \# 3 {.scrollable visibility="uncounted" .smaller}
